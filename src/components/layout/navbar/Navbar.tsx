...
export function Navbar() {
    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-foreground px-6 py-4 shadow-lg">
            <div className="max-w-7xl mx-auto flex items-center gap-12">
                ...
                <div className="hidden lg:flex items-center gap-6 shrink-0">
                    ...
                    <IconButton href="/cliente/wishlist" icon={FaHeart} label="Favoritos" />
            ...
            {menuOpen && (
                <div className="lg:hidden bg-foreground px-6 py-6 flex flex-col gap-6 border-t border-primary-foreground/20">
                    ...
                    <div className="flex flex-col gap-4 text-background">
                        <Link href="/cart" className="flex items-center gap-3 hover:opacity-80" onClick={() => setMenuOpen(false)}>
                            <FaShoppingCart size={20} /><span className="text-sm font-medium">Carrito</span>
                        </Link>
                        <Link href="/cliente/wishlist" className="flex items-center gap-3 hover:opacity-80" onClick={() => setMenuOpen(false)}>
                            <FaHeart size={20} /><span className="text-sm font-medium">Favoritos</span>
                        </Link>
                        <Link href="/login" className="flex items-center gap-3 hover:opacity-80" onClick={() => setMenuOpen(false)}>
                            <FaUser size={20} /><span className="text-sm font-medium">Iniciar Sesión</span>
                        </Link>
                    </div>
                </div>
            )}
