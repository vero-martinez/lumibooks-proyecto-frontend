/**
 * Dialog de detalle de pedido para el panel gestor.
 */
"use client";
...
import type { ManagerOrderDetailResponse } from "@/features/orders/types";
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto p-0 gap-0">
        ...
        <DialogHeader className="bg-primary text-primary-foreground px-6 py-6 space-y-1.5">
          <DialogTitle className="text-xl font-bold text-primary-foreground pr-8">
            {MANAGER_ORDERS.detailTitle} #{order.orderNumber}
          </DialogTitle>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 pt-2 text-xs text-primary-foreground/80">
            <span className="flex items-center gap-1.5">
              ...
              {MANAGER_ORDERS.created}: {formatDateTime(order.createdAt)}
            </span>
            <span className="flex items-center gap-1.5">
              <FaRegClock className="size-3" aria-hidden="true" />
              {MANAGER_ORDERS.updated}: {formatDateTime(order.updatedAt)}
            </span>
          </div>
        </DialogHeader>
        ...
        <div className="px-6 py-8">
          <div className="space-y-10 text-sm">
            ...
            <section>
              <SectionTitle>{MANAGER_ORDERS.recipientData}</SectionTitle>
            ...
            {/* Productos */}
            <section>
              <SectionTitle>{MANAGER_ORDERS.products}</SectionTitle>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {MANAGER_ORDERS.quantityLabel} {item.quantity} &times;{" "}
            ...
            {/* Resumen de costos */}
            <section>
              <SectionTitle>{MANAGER_ORDERS.summary}</SectionTitle>
